...
class DeathClockEngine:
    ...
    def process_nutrition(self, diet_choice, years_penalty, penalty_sugar, penalty_water, nutrition_data):
        # 1. Pobieramy HR diety (np. 1.05)
        diet_hr = nutrition_data['nutrition_data']['diet_patterns'][diet_choice]['hr']

        # 2. Cukier powinien zwiększać HR (ryzyko), a nie być sztywną liczbą lat
        # Upewnij się, że penalty_sugar to mała wartość (np. 0.05), a nie np. 5 lat!
        total_hr = diet_hr + penalty_sugar

        # 3. Obliczamy wpływ na podstawie HR
        # impact = (pozostałe lata / ryzyko) - pozostałe lata
        impact = (self.base_years / total_hr) - self.base_years

        # 4. Korekta o historię i wodę
        # UWAGA: years_penalty i penalty_water powinny być liczbami lat (np. -2.5 lub +1.0)
        # Jeśli years_penalty z funkcji years_history jest dodatnie (np. 2.0), to tu odejmujemy

        finally_impact = impact - abs(years_penalty) + penalty_water

        # DODAJ ZABEZPIECZENIE (Cap):
        # Dieta nie może skrócić życia o więcej niż np. 20 lat i wydłużyć o więcej niż 5
        finally_impact = max(min(finally_impact, 5), -25)

        self.adjustments.append(finally_impact)
        return round(finally_impact, 2)

    def process_stimulants(self, stimulants_choice, stimulants_dosages, stimulants_data):

        hr_sources = stimulants_data['caffeine_data']['sources'][stimulants_choice]['hr_modifier']

        hr_dosages = stimulants_data['caffeine_data']['dosages'][stimulants_dosages]['multiplier']

        total_hr = 1.0 + (hr_sources * hr_dosages)

        impact = float(self.base_years / total_hr) - self.base_years
